...
data_diabetes |>  
  group_by(Diabetes, grupo_edad) |> 
  summarize(promedio = mean(Glucosa, na.rm = TRUE),
            DE = sd(Glucosa, na.rm = TRUE),
            max_valor_glucosa = max(insulina, na.rm = TRUE))
```
...
data_diabetes |>  
  ...
  summarize(max_imc_diab = max(imc[Diabetes == "Si"], na.rm = TRUE),
            ...
            prom_imc_diab = max(imc[Diabetes == "Si"], na.rm = TRUE),
            prom_imc_no_diab = max(imc[Diabetes == "No"], na.rm = TRUE))
```
Percentiles del nivel de glucosa por grupo de edad solo en participantes con 
diabetes usando la función `filter()`
...
Usando `across()` para multiples estadísticos descriptivos para multiples 
variables
...
```
Necesitaremos el paquete finalfit para recodificar variables categóricas
```{r}
library(finalfit)
```
