...
```{r}
data_artritis_1 |> 
  count(categorias_de_edad) # Cambia de variable categórica
```
...
### paso 6: Transformando valores a valores perdidos

```{r}
# Use na_if() to set a single value to missing mydat_tibble <- mydat_tibble %>%    mutate(CDAI = na_if(CDAI, 999))  # Use case_when() to set multiple values to missing mydat_tibble <- mydat_tibble %>%    mutate(CDAI = case_when(CDAI %in% c(777, 888, 999) ~ as.double(NA),                           TRUE                       ~ CDAI))
```
